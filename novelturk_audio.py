...
def _google_call(text, target, source):
    """Tek bir Google isteği; 'çok fazla istek' (429) gelirse artan beklemeyle tekrar dener."""
    from deep_translator import GoogleTranslator
    last = None
    for attempt in range(6):
        _g_throttle()
        try:
            return GoogleTranslator(source=source or "auto", target=target).translate(text) or ""
        except Exception as e:  # noqa: BLE001
            if type(e).__name__ in ("LanguageNotSupportedException", "NotValidPayload", "NotValidLength"):
                raise _FatalTranslateError(f"{type(e).__name__}: {e}") from e     # tekrar denemek anlamsız
            last = e
...
def translate_paragraphs(paras, target, source, engine, concurrency, retries=3):
    ...
    if engine == "google":
        ...
        for attempt in range(retries):
            try:
                return _translate_google_batch(paras, target, source)
