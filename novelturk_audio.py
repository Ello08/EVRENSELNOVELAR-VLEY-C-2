...
def _translate_google_one(text, target, source):
    ...
    for chunk in split_text(text, 4500):     # Google'ın tek istekte kabul ettiği karakter sınırının altında
        try:
            ...
            t = MyMemoryTranslator(source=source or "autodetect", target=target).translate(chunk)
        out.append(t or "")
    ...
    if fail and fail >= max(1, ok):
        if engine == "google":
            log.error("Çevirinin çoğu/tamamı başarısız oldu. Muhtemel neden: bu ağ/IP'den (ör. GitHub Actions, "
                       "bazı VPS'ler) ücretsiz Google Translate arka ucu engelleniyor olabilir. Çözümler: "
                       "--translate-engine argos (tamamen çevrimdışı) deneyin, ya da --proxy ile ev/rezidansiyel "
                       "bir proxy kullanın. Yukarıdaki '[N] çeviri başarısız: ...' satırları asıl hatayı gösterir.")
        else:
