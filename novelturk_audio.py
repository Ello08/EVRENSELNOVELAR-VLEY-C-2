...
Ortam değişkenleri (hepsi isteğe bağlı)
  ...
  NOVEL_FORMAT (mp3,epub)  NOVEL_TRANSLATE_TO (ör. en)  NOVEL_TRANSLATE_SOURCE (varsayılan 'auto')
              NOVEL_TRANSLATE_ENGINE (google|argos)
  UYARI: kaynak metin İngilizce'yse ve --translate-to tr ise, --translate-source'u ELLEMEYİN (auto kalsın)
         ya da açıkça 'en' verin. Kaynağı hedefle aynı dile sabitlemek (ör. ikisi de 'tr') çeviriyi
         sessiz bir no-op'a çevirir: hata vermez ama metin hiç değişmeden geri döner.
    ...
    def fetch(self, url, referer=None):
        ...
        try:
            ...
            try:                     # tembel yüklenen içerik için sayfayı sonuna kaydır
                ...
                pass
            try:
                # Bazı siteler bölüm metnini JS ile birkaç saniye GECİKMELİ değiştiriyor (networkidle'dan
                # sonra bile) ve o süre boyunca "... güvenli bir şekilde yükleniyor" gibi bir yer tutucu
                # gösteriyor (ör. novelturk.com v3). networkidle/scroll sonrası hâlâ bu metin görünüyorsa,
                # gerçek içerik gelene kadar (en fazla 15 sn) bekle; hiç kaybolmazsa zaman aşımıyla devam et.
                page.wait_for_function(
                    "() => !/güvenli bir şekilde yükleniyor|bölümler yükleniyor|sonraki bölüm yükleniyor/i"
...
_BUILTIN_NOISE = [
    # JS ile sonradan doldurulan (lazy-load) sitelerde geçici olarak görünen "yükleniyor" metinleri.
    # Kelime "yükleniyor" tek başına gerçek hikaye metninde de geçebilir (ör. sistem/oyun temalı
    # LitRPG romanlarında "Sistem yükleniyor..." gibi), o yüzden yalnızca bu tam/uzun kalıplar atılır.
    re.compile(r"bölüm içeriği güvenli bir şekilde yükleniyor", re.I),
    re.compile(r"^(bölümler|sonraki bölüm|önceki bölüm) yükleniyor\.*$", re.I),
    re.compile(r"^(loading chapter|please wait while.{0,20}loads?)\.*$", re.I),
]
...
def clean_lines(raw, title="", noise=()):
    ...
    noise = list(noise) + _BUILTIN_NOISE
    out, prev = [], None
    for ln in raw:
        ln = clean_line(ln)
    ...
    try:
        ...
        while cur:
            ...
            if guessed and page.ok and not (_CHAP_TXT.search(fold(page.title)) or page.back):
                ...
                cur = None
                break
            # Bazı siteler engellediğinde/hız sınırladığında 403/429 yerine HTTP 200 ile jenerik, neredeyse
            # boş bir sayfa döndürür (başlık site adından ibaret, içerik yalnızca yoğunluk yedeğiyle ve
            # min_chars'ı zar zor geçerek çıkarılmış, 'sonraki' linki de yok). Bu "sahte başarı" min_chars
            # eşiğini geçtiği için normalde fark edilmez; burada tespit edip GERÇEK bir engelleme/rate-limit
            # gibi ele alıp aşağıdaki backoff+rotasyon mekanizmasına yönlendiriyoruz.
            soft_blocked = (page.ok and not guessed and not nxt and page.method == "yoğunluk"
                            and page.chars < settings.min_chars * 3
                            and fold(page.title or "").strip() in ("", fold(_host(cur))))
            if (not page.ok or soft_blocked) and not guessed:
                # Gerçek bir 'Sonraki' bağlantısını takip ettik ama içerik çıkarılamadı (ya da yukarıdaki
                # gibi "sahte başarı" tespit edildi): bu genelde sitenin geçici olarak sizi kısıtlaması
                # (rate-limit) ya da bir WAF/bot sayfası demektir, gerçekten serinin bittiği anlamına GELMEZ.
                # Backoff ile birkaç kez daha dene.
                ok_retry = False
                for attempt in range(1, 4):
                    wait = min(2 ** (attempt + 2), 60) + random.uniform(0, 3)
                    log.warning("[%d] içerik çıkarılamadı/şüpheli (muhtemel geçici engel/rate-limit); %.0fsn "
                                "sonra tekrar denenecek (%d/3): %s", idx, wait, attempt, cur)
                    time.sleep(wait)
                    try:
                    ...
                    try:
                        ...
                        continue
                    soft_blocked = (page.ok and not nxt and page.method == "yoğunluk"
                                    and page.chars < settings.min_chars * 3
                                    and fold(page.title or "").strip() in ("", fold(_host(cur))))
                    if page.ok and not soft_blocked:
                        digest = content_digest(page.lines)
                        ok_retry = True
                        break
                if not ok_retry:
...
def mymemory_locale(code):
    ...
    Otomatik algılama için deep_translator'ın MyMemoryTranslator'ı 'autodetect' değil düz 'auto' bekler."""
    c = (code or "").strip()
...
class _FatalTranslateError(RuntimeError):
    """Tekrar denemenin anlamsız olduğu çeviri hatası (ör. desteklenmeyen dil kodu)."""


# Google (deep_translator) küçük harfli kod ister ('tr'); bazı diller özel yazılır.
_GOOGLE_CODE_FIX = {"zh": "zh-CN", "zh-cn": "zh-CN", "zh-tw": "zh-TW", "he": "iw", "jv": "jw"}
_EN_WORDS = {"the", "and", "was", "that", "with", "his", "her", "for", "had", "not", "you", "but", "this", "they"}


def normalize_lang(code, engine="google"):
    """'Tr', ' TR ', 'tr_TR', 'pt-br' gibi girdileri motorun beklediği koda çevirir ('auto' korunur)."""
    c = (code or "").strip().replace("_", "-").lower()
    if c in ("", "auto", "autodetect"):
        return "auto"
    if engine == "google":
        return _GOOGLE_CODE_FIX.get(c, c)
    return c.split("-")[0]          # argos: 'tr-TR' -> 'tr', 'pt-br' -> 'pt'


def _detect_source_for_argos(files):
    """Argos 'auto' bilmez. İlk bölümün örneğinden İngilizce mi diye bakar; değilse net hata verir."""
    for _, path in files[:1]:
        sample = path.read_text(encoding="utf-8")[:4000].lower()
        words = re.findall(r"[a-z']+", sample)
        if words and sum(w in _EN_WORDS for w in words) / len(words) > 0.06:
            return "en"
    raise _FatalTranslateError("Argos kaynak dili otomatik algılayamaz. --translate-source ile gerçek dili "
                               "verin (ör. en). GitHub Actions'ta 'translate_source' alanını doldurun.")


def prepare_translate_args(args):
    """Dil kodlarını normalize eder; motor google ise kodları istek atmadan ÖNCE doğrular (hızlı hata)."""
    eng = args.translate_engine
    args.translate_to = normalize_lang(args.translate_to, eng)
    args.translate_source = normalize_lang(args.translate_source, eng)
    if eng == "gemini":
        args.translate_to = normalize_lang(args.translate_to, "argos")          # 'tr-TR' -> 'tr'
        args.translate_source = normalize_lang(args.translate_source, "argos")
        if not os.getenv("GEMINI_API_KEY", "").strip():
            raise _FatalTranslateError(
                "GEMINI_API_KEY tanımlı değil. https://aistudio.google.com/apikey adresinden ücretsiz anahtar alın "
                "ve GitHub'da Settings > Secrets and variables > Actions > New repository secret ile "
                "GEMINI_API_KEY adıyla ekleyin.")
    if eng == "google":
        from deep_translator import GoogleTranslator
        try:
            GoogleTranslator(source=args.translate_source, target=args.translate_to)
        except Exception as e:  # noqa: BLE001
            raise _FatalTranslateError(
                f"Google bu dil kodunu tanımıyor (kaynak='{args.translate_source}', hedef='{args.translate_to}'): "
                f"{type(e).__name__}. Küçük harfli ISO kodu kullanın (ör. tr, en, de, fr, es).") from e


_G_LOCK = threading.Lock()
_G_LAST = [0.0]
_G_MIN_INTERVAL = 0.7        # istekler arası en az bekleme (sn) -> Google limiti ~5 istek/sn, biz ~1.4/sn'deyiz
_G_BATCH_CHARS = 4000        # tek istekte gönderilecek en fazla karakter (Google sınırı 5000)


def _g_throttle():
    """Tüm thread'ler için ortak hız sınırı."""
    with _G_LOCK:
        wait = _G_LAST[0] + _G_MIN_INTERVAL - time.monotonic()
        if wait > 0:
            time.sleep(wait)
        _G_LAST[0] = time.monotonic()
...
def _google_proxies():
    """NOVEL_PROXY (virgülle ayrılmış olabilir) doluysa deep_translator'a proxy olarak verir."""
    raw = os.getenv("NOVEL_PROXY", "").strip()
    if not raw:
        return None
    px = random.choice([x.strip() for x in raw.split(",") if x.strip()])
    return {"http": px, "https": px}


def _google_call(text, target, source):
    """Tek bir Google isteği. 429 gelirse artan beklemeyle dener; kalıcıysa IP engeli sayıp durur
    (hiç başarılı istek yoksa 3 denemede, varsa 6 denemede) — boşuna dakikalarca beklemesin."""
    from deep_translator import GoogleTranslator
    last = None
    for attempt in range(6):
        _g_throttle()
        try:
            res = GoogleTranslator(source=source or "auto", target=target,
                                   proxies=_google_proxies()).translate(text) or ""
            _G_STATE["ok"] += 1
            return res
        except Exception as e:  # noqa: BLE001
            name = type(e).__name__
            if name in ("LanguageNotSupportedException", "NotValidPayload", "NotValidLength"):
                raise _FatalTranslateError(f"{name}: {e}") from e     # tekrar denemek anlamsız
            last = e
            if name == "TooManyRequests" and attempt + 1 >= (6 if _G_STATE["ok"] else 3):
                raise _FatalTranslateError(
                    "Google bu IP'den gelen istekleri engelliyor (TooManyRequests, bekleyince geçmiyor). "
                    "GitHub Actions gibi veri merkezi IP'lerinde olağandır. Çözümler: translate_engine=argos "
                    "seçin; ya da çeviriyi kendi bilgisayarınızda çalıştırın; ya da NOVEL_PROXY secret'ına "
                    "bir proxy yazın.") from e
            wait = min(5 * 2 ** attempt, 90) + random.random()      # 5, 10, 20, 40, 80, 90 sn
            log.warning("Google çeviri hatası (%s), %.0f sn beklenip tekrar denenecek (%d/6)",
                        name, wait, attempt + 1)
            time.sleep(wait)
    raise RuntimeError(str(last))


def _translate_google_batch(paras, target, source):
    """Paragrafları ~4000 karakterlik gruplara birleştirip TEK istekle çevirir (yüzlerce istek yerine
    bölüm başına birkaç istek). Satır sayısı tutmazsa o grup paragraf paragraf çevrilir."""
    groups, cur, size = [], [], 0
    for p in paras:
        if cur and size + len(p) + 1 > _G_BATCH_CHARS:
            groups.append(cur)
            cur, size = [], 0
        cur.append(p)
        size += len(p) + 1
    ...
    out = []
    for g in groups:
        if len(g) == 1 and len(g[0]) > _G_BATCH_CHARS:            # tek dev paragraf: cümle sınırından böl
            out.append(" ".join(_google_call(c, target, source) for c in split_text(g[0], _G_BATCH_CHARS)).strip())
            continue
        res = _google_call("\n".join(g), target, source).split("\n")
        res = [r.strip() for r in res if r.strip()]
        if len(res) == len(g):
            out.extend(res)
        else:                                                      # satırlar kaydı: güvenli yedek
            log.debug("Toplu çeviride satır sayısı tutmadı (%d != %d), paragraf paragraf çevriliyor", len(res), len(g))
            out.extend(_google_call(p, target, source) for p in g)
    return out


_GEMINI_LANG = {"tr": "Turkish", "en": "English", "de": "German", "fr": "French", "es": "Spanish", "it": "Italian",
                "pt": "Portuguese", "ru": "Russian", "ja": "Japanese", "ko": "Korean", "zh": "Chinese",
                "ar": "Arabic", "nl": "Dutch", "pl": "Polish", "id": "Indonesian", "vi": "Vietnamese"}
_GEMINI_STATE = {"last": 0.0}
_GEMINI_LOCK = threading.Lock()
_GEMINI_CHUNK_CHARS = 6000


def _gemini_throttle():
    """Ücretsiz katman dakikada ~10 istek; varsayılan 7 sn aralık (GEMINI_MIN_INTERVAL ile değişir)."""
    gap = float(os.getenv("GEMINI_MIN_INTERVAL") or "7")          # Actions boş değişkeni "" olarak verir
    with _GEMINI_LOCK:
        wait = _GEMINI_STATE["last"] + gap - time.monotonic()
        if wait > 0:
            time.sleep(wait)
        _GEMINI_STATE["last"] = time.monotonic()


def _gemini_request(system, user):
    """Tek Gemini isteği (urllib; ek paket gerekmez). 429/5xx için bekleyip dener, kalıcı hatada durur."""
    ...
    gen = {"temperature": 0.3, "maxOutputTokens": 16384}
    if "2.5-flash" in model:
        gen["thinkingConfig"] = {"thinkingBudget": 0}        # düşünme token'ı harcamasın, çeviri yeter
    body = {
        ...
        "safetySettings": [{"category": c, "threshold": "BLOCK_NONE"} for c in (
            ...
            "HARM_CATEGORY_SEXUALLY_EXPLICIT", "HARM_CATEGORY_DANGEROUS_CONTENT")],   # roman şiddet içerebilir
    }
    data = json.dumps(body).encode("utf-8")
    last = None
    for attempt in range(6):
        _gemini_throttle()
        req = urllib.request.Request(f"{base}/models/{model}:generateContent", data=data, method="POST",
        ...
        try:
            ...
            if not cands:
                raise RuntimeError(f"Gemini boş yanıt döndürdü: {str(resp.get('promptFeedback') or resp)[:200]}")
            parts = (cands[0].get("content") or {}).get("parts") or []
            return "".join(p_.get("text", "") for p_ in parts)
        except urllib.error.HTTPError as e:
            detail = e.read().decode("utf-8", "replace")[:400]
            if e.code in (400, 401, 403, 404):
                raise _FatalTranslateError(
                    f"Gemini HTTP {e.code}: {detail}  (GEMINI_API_KEY secret'ı ve GEMINI_MODEL adını kontrol edin)") from e
            if e.code == 429 and ("PerDay" in detail or "per day" in detail.lower()):
                raise _FatalTranslateError(
                    "Gemini günlük ücretsiz kotası doldu. Yarın tekrar çalıştırın ya da GEMINI_MODEL'i değiştirin.") from e
            last = f"HTTP {e.code}: {detail[:200]}"
        except Exception as e:  # noqa: BLE001
            last = f"{type(e).__name__}: {e}"
        wait = min(20 * (attempt + 1), 100) + random.random()
        log.warning("Gemini hatası (%s), %.0f sn sonra tekrar denenecek (%d/6)", last, wait, attempt + 1)
        time.sleep(wait)
    raise RuntimeError(f"Gemini yanıt vermedi: {last}")


def _gemini_system_prompt(target, source):
    tgt = _GEMINI_LANG.get(target, target)
    src = "the source language (detect it)" if source in ("", "auto") else _GEMINI_LANG.get(source, source)
    return (f"You are an expert literary translator working on a web novel. Translate from {src} into {tgt}. "
            f"Write fluent, natural, well-edited {tgt} in a literary novel style; never translate word by word. "
            "Keep character names, place names and invented terms (spells, ranks, titles) consistent and "
            "untranslated unless they have an established translation. "
            "The input is a list of paragraphs, each starting with a marker like [[12]]. "
            "Return EXACTLY one translated paragraph for every input paragraph, each starting with the SAME marker "
            "[[n]], in the same order. Never merge, split, skip or add paragraphs. Translate everything, including "
            "system messages and dialogue, and do not add notes, explanations or code fences.")


def _gemini_parse(text, wanted):
    found = {}
    for m in re.finditer(r"\[\[(\d+)\]\]\s*(.*?)(?=\s*\[\[\d+\]\]|\Z)", text, flags=re.S):
        n = int(m.group(1))
        if n in wanted and m.group(2).strip():
            found.setdefault(n, " ".join(m.group(2).split()))
    return found


def _translate_gemini_batch(paras, target, source):
    """Paragrafları ~6000 karakterlik gruplar halinde numaralı işaretlerle gönderir; eksik dönenleri tek tek yeniden ister."""
    system = _gemini_system_prompt(target, source)
    groups, cur, size = [], [], 0
    for i, p_ in enumerate(paras):
        if cur and size + len(p_) > _GEMINI_CHUNK_CHARS:
            groups.append(cur)
            cur, size = [], 0
        cur.append(i)
        size += len(p_) + 8
    ...
    out = {}
    for g in groups:
        user = "\n\n".join(f"[[{i}]] {paras[i]}" for i in g)
        got = _gemini_parse(_gemini_request(system, user), set(g))
        for i in [i for i in g if i not in got]:                   # eksikleri tek tek dene
            for _ in range(2):
                got.update(_gemini_parse(_gemini_request(system, f"[[{i}]] {paras[i]}"), {i}))
                if i in got:
                    break
            if i not in got:
                raise RuntimeError(f"Gemini {i}. paragrafı çeviremedi")
        out.update(got)
    return [out[i] for i in range(len(paras))]
...
def translate_paragraphs(paras, target, source, engine, concurrency, retries=3):
    ...
    if engine in ("google", "gemini"):
        batch_fn = _translate_gemini_batch if engine == "gemini" else _translate_google_batch
        last = None
        for attempt in range(retries):
            try:
                return batch_fn(paras, target, source)
            except _FatalTranslateError:
                raise
            except Exception as e:  # noqa: BLE001
                last = e
                time.sleep(min(10 * (attempt + 1), 30))
        raise RuntimeError(f"çeviri başarısız: {last}")
    fn = _translate_argos_one
    workers = 1
...
def translate_series_dir(src_dir, out_dir, args):
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    ok = fail = 0
    if args.translate_engine == "argos" and args.translate_source == "auto":
        args.translate_source = _detect_source_for_argos(files)
        log.info("Argos kaynak dil (otomatik tahmin): %s", args.translate_source)
    ...
    for idx, path in files:
        ...
        try:
            allp = ([title] if title else []) + paras
            res = translate_paragraphs(allp, args.translate_to, args.translate_source,
                                       args.translate_engine, args.translate_concurrency)
            t_title, t_paras = (res[0], res[1:]) if title else (title, res)
...
def _warn_if_translate_broken(ok, fail, engine):
    ...
    if fail and fail >= max(1, ok):
        if engine == "google":
            log.error("Çevirinin çoğu/tamamı başarısız oldu. Google ücretsiz ucu bu IP'den engelli olabilir "
                       "(GitHub Actions'ta sık görülür). Çözümler: --translate-engine argos (çevrimdışı), "
                       "kendi bilgisayarınızda çalıştırmak ya da NOVEL_PROXY ile proxy kullanmak. "
                       "Yukarıdaki '[N] çeviri başarısız: ...' satırları asıl hatayı gösterir.")
        elif engine == "gemini":
            log.error("Çevirinin çoğu/tamamı başarısız oldu. GEMINI_API_KEY, GEMINI_MODEL ve günlük kotayı "
                       "kontrol edin. Yukarıdaki '[N] çeviri başarısız: ...' satırları asıl hatayı gösterir.")
        else:
...
def cmd_translate(args):
    ...
    elif args.translate_engine == "argos":
        _import("argostranslate", "argostranslate")
    try:
        prepare_translate_args(args)
    except _FatalTranslateError as e:
        log.error("%s", e)
        return 2
    src = args.translate_source
    if src != "auto" and src == args.translate_to:
        log.error("--translate-source ('%s') ile --translate-to ('%s') aynı dil; bu çeviriyi anlamsız bir "
                   "no-op'a çevirir (Google metni değiştirmeden geri döndürür, hata da vermez). Kaynak "
                   "içeriğin GERÇEK dilini verin (ör. metin İngilizce'yse --translate-source en) ya da "
                   "otomatik algılama için tamamen kaldırın/--translate-source auto kullanın.",
                   args.translate_source, args.translate_to)
        return 2
    if args.translate_engine == "google":
        _import("deep_translator", "deep-translator")
    elif args.translate_engine == "argos":
    ...
    for d in dirs:
        try:
            ...
            log.error("Çeviri durduruldu: %s", e)
            return 2
        ok, fail = ok + o, fail + f
...
def parse_args(argv=None):
    ...
    p.add_argument("--translate-source", default=os.getenv("NOVEL_TRANSLATE_SOURCE", "auto"), metavar="DİL",
                   help="Kaynak dil kodu (varsayılan 'auto': otomatik algılar; Argos'ta 'auto' yerine gerçek "
                        "bir kod gerekir, ör. 'en'). DİKKAT: kaynağı hedefle AYNI dile sabitlemeyin (ör. "
                        "kaynak da hedef de 'tr' olursa) — o zaman Google hiçbir şey yapmadan metni olduğu "
                        "gibi geri döndürür ('çeviri' sessizce no-op olur, hata da vermez).")
    p.add_argument("--translate-engine", choices=["google", "argos", "gemini"],
...
def main(argv=None):
    ...
    if not args.verbose:
        # argostranslate root logger'a her cümle için ('tokenized', [...]) gibi tuple log'ları basar; log'u boğar
        class _DropTupleLogs(logging.Filter):
            def filter(self, record):
                return not (record.name == "root" and isinstance(record.msg, tuple))
        for _h in logging.getLogger().handlers:
            _h.addFilter(_DropTupleLogs())
        for _n in ("stanza", "argostranslate", "ctranslate2"):
